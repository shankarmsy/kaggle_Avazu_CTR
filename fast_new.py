...
submission = 'Sub-iter1.csv'  # path of to be outputted submission file
    ...
    for t, date, ID, x, y in data(train, D):  # data is a generator
        ...
        if breakdt == date:
           ...
           rows.append(x)
            ...
            outfile=open('train'+str(breakdt)+'.csv', 'w')
            pickle.dump(rows, outfile)
            print('File for day %d finished, elapsed time: %s' % (
                     date, str(datetime.now() - start)))
            breakdt=date                    
...
# with open('train'+str(date)+'.csv', 'w') as outfile:
#                 print('File for day %d finished, elapsed time: %s' % (
#                     date, str(datetime.now() - start)))
#                 for row in rows:
#                     # print row
