...
def define_params(algo):
    ...
    if algo=='nn':
        vw_params = {'-nn':5, 'l':10^10, '-l2':10^10,'-l1':10^10}
        ranges={'-nn':'5 20', 'l':'-L 1e-10 20', '-l2':'-L 1e-10 20',
                '-l1':'-L 1e-10 10'}
        param_seq =['-nn','l', '-l2', '-l1']
        return vw_params, ranges, param_seq

    if algo=='logi':
        vw_params = {'l':0.132217, '-l2':1.3191e-10,'-l1':1.04011e-10}
        ranges={'l':'1e-10 20', '-l2':'1e-10 20','-l1':'1e-10 10'}
        param_seq =['l', '-l2', '-l1']
        return vw_params, ranges, param_seq

    if algo=='ftrl':
        vw_params = {'-ftrl_alpha':0.0734475, '-ftrl_beta':0.118585, '-l2':1.3191e-10,'-l1':1.04011e-10}
        ranges={'-ftrl_alpha':'-L 1e-10 10', '-ftrl_beta':'-L 1e-10 10','-l2':'-L 1e-10 20', '-l1':'-L 1e-10 10'}
...
cmd_vwh='vw-hypersearch -t {} -L '.format(cvalfile)
...
for iter,key in enumerate(param_seq):
    ...
    cmd=cmd_vw.format(loss_function, algo_parm, trainfile, 1, interactions)
    params=''
...
cmd+=params+' -f {} --holdout_period 100'.format(final_model)
os.system('{} 2>&1 | tee {}'.format(cmd, logfile))
print cmd
